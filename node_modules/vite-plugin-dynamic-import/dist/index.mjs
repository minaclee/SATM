...
function mappingPath(paths, alias) {
  ...
  for (const p of paths) {
    ...
    const ext = path$a.extname(importee);
    maps[p] = [
...
class Resolve {
  ...
  async tryResolveAlias(importee, importer) {
    ...
    const relativePath = alias.replacement.startsWith(".") ? alias.replacement : relativeify_1(path$a.posix.relative(path$a.dirname(importer), alias.replacement));
    const resolvedAlias = {
      ...alias,
      findString,
  ...
  tryResolveBare(importee, importer) {
    ...
    const node_modules = path$a.join(this.config.root, "node_modules");
    let level = "";
    let find, replacement;
    let p;
    while (p = paths.shift()) {
      level = path$a.posix.join(level, p);
      const fullPath = path$a.join(node_modules, level);
      if (require$$0$2.existsSync(fullPath)) {
        find = level;
        const relativePath = relativeify_1(path$a.posix.relative(path$a.dirname(importer), node_modules));
    ...
    const alias = {
      ...
      replacement,
      findString: find,
      relative: replacement.startsWith(".") ? replacement : relativeify_1(path$a.posix.relative(path$a.dirname(importer), replacement))
    };
    return {
      type: "bare",
  ...
  resolveAlias(importee, importer, alias) {
    ...
    if (replacement.startsWith(".")) {
      ...
      const relativePath = relativeify_1(path$a.posix.relative(
        path$a.dirname(importer),
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (/node_modules\/(?!\.vite\/)/.test(id))
        return;
      if (!extensions.includes(path$a.extname(id)))
        return;
      ...
      await walk_1(ast, {
        async ImportExpression(node) {
          ...
          files = files.filter((f) => path$a.join(path$a.dirname(id), f) !== id);
...
async function globFiles(node, code, importer, resolve, extensions, loose = true) {
  ...
  const fileGlobs = globs.map(
    (g) => path$a.extname(g) ? g : g + `.{${extensions.map((e) => e.replace(/^\./, "")).join(",")}}`
  );
  files = out.sync(fileGlobs, { cwd: path$a.dirname(importer) }).map((file) => relativeify_1(file));
